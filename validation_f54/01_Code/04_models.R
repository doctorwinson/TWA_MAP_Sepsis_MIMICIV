...
raw<-merge(raw,sv[,.(stay_id,both_pair_by24)],by="stay_id",all.x=TRUE)
...
run<-function(label,covariates,dds=dl,filter=NULL,exposure="map_q",save=FALSE){
 ...
 if(any(vapply(fits,function(f)any(!is.finite(coef(f)))||any(!is.finite(vcov(f))),logical(1)))||any(grepl("infinite|converge",ww)))return(data.table(model=label,term="map_qQ4",status=paste("unstable",paste(unique(ww),collapse=";"))))
